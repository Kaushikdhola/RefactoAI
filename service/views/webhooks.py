        ...
        try:
            ...
            if event == 'push':
                try:
                    ...
                    return Response({'status': 'error', 'message': 'Key missing in payload message'}, status=status.HTTP_400_BAD_REQUEST)
                ...
                try:
                    ...
                    return Response({'status': 'error', 'message': 'Error in fetching commit data'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
                ...
                if response_commit.status_code == 200:
                    try:
                        ...
                        return Response({'status': 'error', 'message': 'Failed to decode JSON response'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            return Response({'status': 'error', 'message': 'Event type is Invalid!!!!!'}, status=status.HTTP_400_BAD_REQUEST)
